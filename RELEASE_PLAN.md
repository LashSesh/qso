...
The release process is **automated via GitHub Actions**. When a version tag is pushed, the workflow automatically:

1. **Builds Rust Crate:**
   - Compiles release binary
   - Runs final validation tests
   - Validates `cargo publish --dry-run`
   - Generates rustdoc documentation
   - Packages artifacts as tarball

2. **Builds Python Wheels:**
   - Cross-platform builds (Linux, macOS, Windows)
   - Multiple Python versions (3.10, 3.11, 3.12)
   - Uses `maturin` for wheel generation
   - Uploads all wheels as artifacts

3. **Creates GitHub Release:**
   - Automatically extracts version from tag
   - Reads release notes from `CHANGELOG.md`
   - Attaches Rust tarball and Python wheels
   - Publishes release (non-draft)

4. **Optional Docker Build:**
   - Builds Docker image (if `vars.DOCKER_BUILD_ENABLED == 'true'`)
...
# Monitor at: https://github.com/LashSesh/qso/actions/workflows/release.yml
```
...
**Artifacts Generated:**
- `rust-crate/metatron-qso-rs-release.tar.gz`
- `python-wheel-ubuntu-latest-py3.10/*.whl`
- `python-wheel-ubuntu-latest-py3.11/*.whl`
- `python-wheel-ubuntu-latest-py3.12/*.whl`
- `python-wheel-macos-latest-py3.10/*.whl`
- `python-wheel-macos-latest-py3.11/*.whl`
- `python-wheel-macos-latest-py3.12/*.whl`
- `python-wheel-windows-latest-py3.10/*.whl`
- `python-wheel-windows-latest-py3.11/*.whl`
- `python-wheel-windows-latest-py3.12/*.whl`
- `docker-image/qdash-docker-image.tar.gz` (optional)

**Post-Automation Manual Steps:**
After the automated workflow completes, maintainers should:
1. Review the generated GitHub Release
2. Optionally publish Rust crate: `cd metatron-qso-rs && cargo publish`
3. Optionally publish Python wheels to PyPI: `cd metatron_qso_py && maturin publish`
4. Announce release (see Communication Plan below)
...
**Automated via CI** - See "Automated Release CI" above.

**Manual Steps (if needed):**
1. Create git tag: `git tag -a v0.1.0 -m "Release v0.1.0"`
2. Push tag: `git push origin v0.1.0`
3. CI automatically creates GitHub Release at https://github.com/LashSesh/qso/releases
4. Review and verify release artifacts
5. Edit release notes if needed (CI uses CHANGELOG.md)
