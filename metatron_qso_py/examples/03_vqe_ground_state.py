...
def main():
    ...
    for ansatz_type, ansatz_name in ansatz_types:
        ...
        print(f"  Relative error:            {(result['error'] / abs(result['classical_ground_energy']) * 100):.6f}%")
        print(f"  Iterations:                {result['iterations']}")
        print()

        # Show first few probabilities
        final_probs = result['final_state']
        print(f"  Final state (first 5 nodes):")
    ...
    for name, result in results:
        print(f"{name:<30} {result['ground_state_energy']:>15.10f} "
