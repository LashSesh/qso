...
use apollyon_mef_bridge::trichter::{
    FunnelGraph, GuidanceVector, HDAGField, Hyperbion, Policy, PolicyParams,
    State5D as TrichterState5D,
};
...
impl InterlockAdapter {
    ...
    pub fn evaluate_gate(
        ...
        // Create PoR
        let por_valid =
            delta_pi <= self.config.gate_delta_pi_max && phi >= self.config.gate_phi_threshold;
    ...
    pub fn condense(&mut self, state: &TrichterState5D, guidance: &[f64; 5]) -> TrichterState5D {
        if self.config.enable_funnel_ops {
            ...
            let guidance_vec =
                GuidanceVector::new(guidance[0], guidance[1], guidance[2], guidance[3]);
            ...
            let condensed = if let Some((_, node)) = self.funnel.nodes.iter().max_by(|a, b| {
                a.1.mass
