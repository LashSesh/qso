...
impl ShadowController {
    ...
    fn check_gate_stability(&self) -> bool {
        ...
        let fire_count = recent
            .iter()
            .filter(|d| matches!(d, GateDecision::FIRE))
            .count();
    ...
    fn check_stability(&self) -> bool {
        ...
        let delta_f_ok = recent
            .iter()
            .all(|m| m.delta_f <= self.criteria.max_delta_f);

        // Coherence should remain high
        let coherence_ok = recent
            .iter()
            .all(|m| m.s_mand >= self.criteria.min_coherence);
    ...
    fn activate(&mut self) {
        tracing::info!(
            "ACTIVATION: Shadow → Active mode after {} windows",
            self.window_passes
        );
