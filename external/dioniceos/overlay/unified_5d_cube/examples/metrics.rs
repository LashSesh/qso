...
use std::path::Path;
use unified_5d_cube::{
    tick_5d_cube, InterlockAdapter, InterlockConfig, MetricsCollector, MetricsFormat,
};
...
fn main() {
    ...
    // Run multiple ticks
    for i in 0..10 {
        let result = tick_5d_cube(&mut adapter, &state, prev_state.as_ref(), i as f64 * 0.1, i);

        // Collect metrics
        collector.add(result.metrics.clone());

        println!(
            "Tick {}: ΔF={:.4}, W2={:.4}, S_mand={:.4}",
