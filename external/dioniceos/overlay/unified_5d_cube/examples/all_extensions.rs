...
fn main() {
    ...
    println!(
        "  ✓ Funnel operations: {} (Policy: {:?})",
        config.enable_funnel_ops, config.funnel_policy
    );
    ...
    for i in 0..12 {
        ...
        println!("\nTick {:2}: ", i);
        println!(
            "  State:     norm={:.4}  [x={:.3}, y={:.3}, z={:.3}, ψ={:.3}, ω={:.3}]",
            next_state.norm(),
        ...
        if let Some(ref vec8) = result.vector_8d {
            ...
            println!(
                "  8D Vector: norm={:.6}  spiral=[{:.3}..{:.3}]  spectral=[{:.3}..{:.3}]",
                norm, vec8[0], vec8[4], vec8[5], vec8[7]
            );
        }

        println!(
            "  Metrics:   ΔF={:.6}  W2={:.6}  S_mand={:.4}  BI={:.4}",
